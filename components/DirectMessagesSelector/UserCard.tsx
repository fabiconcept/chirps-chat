...
export default function UserCard({ 
    ...
    return (
        <div 
            className={cn(
                "flex items-center gap-2 min-[900px]:max-w-xs w-full cursor-pointer transition-[transform,translate,scale,rotate,color,background-color,border-color,outline-color,text-decoration-color,fill,stroke,--tw-gradient-from,--tw-gradient-via,--tw-gradient-to,padding] duration-300 ease-in-out hover:bg-foreground/3 perspective-distant group relative after:absolute after:inset-0 after:w-1.5 after:rounded-r-3xl after:left-0 after:top-1/2 after:-translate-y-1/2 after:h-[80%] after:bg-[#7600c9] after:-translate-x-full after:transition-transform after:duration-300 after:ease-in-out",
                        ...
                        <AnimatePresence mode="wait">
                            {isTyping ? (
                                ...
                                >
                                    ...
                                    <span className="truncate min-w-0 max-sm:max-w-32">{message}</span>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
